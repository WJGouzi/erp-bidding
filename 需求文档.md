...
| 功能点 | 前端交互 | 后端 API | 实现状态 |
|--------|---------|---------|---------|
| 左侧展示 | 系统解析的招标要求（所有字段可编辑），每项旁有编辑图标 | `GET /{task_id}/check-items` | ✅ 已实现 |
...
flowchart TD
    ...
    SelectPkg --> CheckItems["7. GET /api/bidding/tasks/{id}/check-items<br/>读取待核对项"]
    CheckItems --> ConfirmCheck["8. POST /api/bidding/tasks/{id}/check-items/confirm<br/>确认核对结果"]
...
| 核对确认 | `GET /{id}/check-items`、`POST /{id}/check-items/confirm` | 人工修正后写回分析结果，后续生成链路直接消费确认后的内容 |
