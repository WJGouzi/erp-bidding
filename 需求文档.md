...
- 宝塔服务器 创建虚拟环境并安装依赖 ( pip install -r requirements.txt )
# 1. 创建虚拟环境 + 安装依赖
cd /www/wwwroot/erp-bidding/erp-bidding && python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt
# 2. 启动服务（已修正语法错误）
lsof -ti :23112 | xargs kill -9; cd /www/wwwroot/erp-bidding/erp-bidding && source .venv/bin/activate && python3 run.py